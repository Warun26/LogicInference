...
private:
    ofstream outputFile;
public:
    Output(bool inference, string pathToOutputFile="output.txt");
};
...
Output::Output(bool inference, string pathToOutputFile)
{
    char *fileName=new char[pathToOutputFile.size()+1];
    fileName[pathToOutputFile.size()]=0;
    memcpy(fileName,pathToOutputFile.c_str(),pathToOutputFile.size());
    this->outputFile.open(fileName);
    if (inference) outputFile<<"TRUE";
